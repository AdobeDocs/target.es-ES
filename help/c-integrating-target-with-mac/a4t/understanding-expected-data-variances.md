...
Información sobre las variaciones de datos previstas entre [!DNL Target] y Adobe [!DNL Analytics] al *utilizar* y *no* utilizar Analytics como fuente de informes (A 4 T). A4T reduce considerablemente la variación de datos.

## Varianza de datos esperada al utilizar A 4 T {#expected-using-a4t}
...
Estos son algunos escenarios en los que podría experimentar varianza de datos esperada:

* A 4 T permite la posibilidad de que se produzca una visita de Target (parte superior de la página), pero no se produce ninguna visita de Analytics (final de página). Un ejemplo sería si el usuario carga la página, pero cierra el explorador antes de activar la llamada de Analytics. En estos casos, A 4 T excluye la visita de Target de nuestros datos. El motivo es que permitir que las visitas de Target (nuevamente, parte superior de la página) cuenten como visitas de Analytics en ausencia de una llamada real de Analytics crearía incoherencias con el conjunto de datos en Analytics (inflación del visitante, etc.).

   Si se configura una prueba de redireccionamiento en Target para dividir el tráfico 50/50 (o 25/25/25, etc.), es posible que el comportamiento del usuario no se divida uniformemente. Si ve una división desigual, simplemente significa que un grupo de usuarios no ejecutó una llamada de Analytics en la página de aterrizaje más que los demás grupos. Este error al ejecutar la llamada de Analytics para un grupo provocaba la exclusión de la visita de Target para que ese usuario se excluyera.

   Esto es algo que esperamos abordar en el futuro a medida que trabajamos hacia A 4 T en la plataforma de Adobe Experience Platform. Nuestros equipos están trabajando a través de la mejor manera de gestionar estos distintos eventos en diferentes momentos de la página.
