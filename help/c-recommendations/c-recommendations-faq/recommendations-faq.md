...
En la cadena de consulta, puede pasar ID de entidad para las entidades que desee excluir de las recomendaciones. Por ejemplo, quizás desee excluir artículos que ya se encuentren en el carro de compras.
...
>[!NOTE]
>
>Si se excluyen demasiadas entidades, las recomendaciones se comportan como si no hubiera suficientes entidades para rellenar la plantilla de recomendación.

Para excluirlo, `entityIds`anexe el `&excludes=${mbox.excludedIds}` token a la URL de contenido de la oferta. Cuando se extrae la dirección URL de contenido, se substituyen los parámetros requeridos mediante parámetros de solicitud de mbox actuales.

De forma predeterminada, esta característica está habilitada para las recomendaciones recién creadas. Las recomendaciones existentes deben guardarse para admitir entidades dinámicamente excluidas.
