...
* El método recomendado es mediante un administrador de etiquetas como Adobe Launch o Adobe Dynamic Tag Manager (DTM). Consulte la lección [Agregar Adobe Target](https://docs.adobe.com/content/help/en/experience-cloud/implementing-in-websites-with-launch/implement-solutions/target.html) del tutorial [Implementación del Experience Cloud en sitios web con inicio](https://docs.adobe.com/content/help/en/experience-cloud/implementing-in-websites-with-launch/index.html) para obtener más información.
...
Puede evitar el parpadeo utilizando un fragmento de ocultamiento previo que oculta la página (o partes especificadas) y luego lo revela después de at. js y la solicitud global se haya cargado completamente. El fragmento debe añadirse antes de cargar at.js.

Si va a implementar at. js a través de una implementación asíncrona de Launch, asegúrese de incluir el fragmento de ocultamiento previo directamente en las páginas, antes del código de incrustación de lanzamiento, tal como se describe en [la sección Agregar fragmento](https://docs.adobe.com/content/help/en/experience-cloud/implementing-in-websites-with-launch/implement-solutions/target.html#add-the-target-pre-hiding-snippet) de ocultamiento previo de la [implementación del tutorial](https://docs.adobe.com/content/help/en/experience-cloud/implementing-in-websites-with-launch/index.html)de Experience Cloud en Sitios web con Launch.

Si implementa at. js a través de una implementación sincrónica de DTM, el fragmento de ocultamiento previo se puede agregar a través de una regla de carga de página activada en la parte superior de la página.
...
Según cómo utilice [targetglobalsettings](/help/c-implementing-target/c-implementing-target-for-client-side-web/targetgobalsettings.md), es posible que tenga que realizar modificaciones adicionales en el código después de descargar at. js. Por ejemplo, si necesita emplear unas configuraciones que difieran un poco para las implementaciones de [!DNL Target] en varios sitios web y no puede definirlas de forma dinámica mediante JavaScript personalizado, haga las personalizaciones manualmente después de descargar el archivo y antes de cargarlo en el sitio web correspondiente.
...
## ¿Qué doctype HTML requiere at. js?

at. js requiere el doctype HTML 5.

Esta sintaxis es:

`<!DOCTYPE html>`

El doctype HTML 5 garantiza que la página se carga en modo estándar. Cuando se carga en modo quirks, algunas API de JS a las que at. js depende están desactivadas. Target deshabilita at. js en modo quirks.
