...
La siguiente ilustración muestra el encabezado del gráfico de una actividad de ejemplo con los encabezados [!UICONTROL Conversion Rate], [!UICONTROL Lift] y [!UICONTROL Confidence] resaltados.
...
![imagen de detalle de tasa de conversión](assets/conversion-rate-detail.jpg)
...
El *intervalo de confianza* es un rango de estimaciones dentro del cual se puede encontrar el valor verdadero de la métrica en un nivel de confianza determinado. Target siempre muestra intervalos de confianza del 95 %. El intervalo de confianza aparece como un porcentaje positivo o negativo de color gris claro en la columna Tasa de conversión. En el ejemplo siguiente, el intervalo de confianza de alza de la Experiencia B es más o menos 15,65%.

![imagen conversion_rate](assets/conversion_rate.png)

**Ejemplo:** El RPV observado de una experiencia es de 10 $ y su **intervalo de confianza** del 95 % es de 5 a 15 $. Desconocido para nosotros, su verdadero RPV es $12. Entonces, si ejecutamos esta prueba varias veces, el 95 % de las veces el intervalo de confianza que calculamos contendrá el valor _true_ del RPV de 12 $.
...
Para calcular estas métricas calculadas, descargue el archivo de Excel [Calculadora de confianza completa](/help/main/assets/complete_confidence_calculator.xlsx) de Target para especificar el valor de la actividad o revise [cálculos estadísticos en pruebas A/Bn](/help/main/c-reports/statistical-methodology/statistical-calculations.md).
...
## ¿Por qué recomienda [!DNL Target] utilizar las pruebas T de Welch? {#t-test}
...
[!DNL Target] recomienda usar [prueba t de Welch](https://en.wikipedia.org/wiki/Welch%27s_t-test), ya que requieren menos suposiciones que alternativas como las pruebas z, y son la prueba estadística adecuada para hacer comparaciones por pares de métricas comerciales (cuantitativas) entre experiencias de control y experiencias alternativas.
...
Dada esta normalidad de la media, se puede demostrar que la estadística de prueba que se va a utilizar sigue una distribución t, porque es la relación entre un valor distribuido normalmente (la diferencia en las medias de la métrica empresarial) y un término de escala basado en una estimación de los datos (el error estándar de la diferencia en las medias). La **prueba t** es entonces la prueba de hipótesis apropiada, dado que la estadística de prueba sigue una distribución t.

### Por qué no se utilizan otras pruebas

Una **prueba Z** es técnicamente inapropiada porque, en el escenario típico de las pruebas A/B, el denominador de la estadística de prueba no se deriva de una varianza conocida y, en cambio, debe estimarse a partir de los datos. Sin embargo, para tamaños de muestra lo suficientemente grandes, la prueba z y la prueba t son idénticas.

No se utilizan **pruebas chi cuadrado** porque son apropiadas para determinar si existe una relación cualitativa entre dos variantes (es decir, una hipótesis nula de que no hay diferencia entre las variantes). Las pruebas T son más apropiadas para el escenario de _cuantitativamente_ comparando métricas.
