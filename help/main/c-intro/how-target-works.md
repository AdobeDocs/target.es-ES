...
description: Descubra cómo [!DNL Adobe Target] funciona, incluida información sobre bibliotecas de JavaScript (AEP Web SDK at.js), centros de datos de Adobe, pruebas de SEO y bots.
...
# Cómo [!DNL Adobe Target] works

Descubra cómo [!DNL Adobe Target] funciona, incluida información sobre las bibliotecas de JavaScript ([!DNL Adobe Experience Platform Web SDK] y at.js). Este artículo también presenta los distintos tipos de actividades que puede crear mediante [!DNL Target]. También puede obtener información sobre la red de Edge de [!DNL Target], la optimización de los motores de búsqueda (SEO) y cómo [!DNL Target] detecta los bots.

## [!DNL Adobe Target] Bibliotecas de JavaScript {#libraries}

[!DNL Target] se integra con los sitios web que utilizan la variable [!DNL Experience Platform Web SDK] o at.js:

* **[!DNL Adobe Experience Platform Web SDK]:** La variable [SDK web de Experience Platform](https://developer.adobe.com/target/implement/client-side/aep-web-sdk/){target=_blank} es una nueva biblioteca JavaScript del lado del cliente. La variable [!DNL Experience Platform Web SDK] permite a los clientes de [!DNL Adobe Experience Cloud] interactúe con los distintos servicios de la [!DNL Experience Cloud] (incluido [!DNL Target]) a través de la función [!DNL Experience Platform] Red perimetral. [!DNL Adobe] recomienda que todos los clientes nuevos de [!DNL Target] implementen el [!DNL Experience Platform Web SDK].
...
>[!NOTE]
>
>La biblioteca mbox.js es una biblioteca de implementación heredada para [!DNL Target]. La biblioteca mbox.js deja de ser compatible a partir del 31 de marzo de 2021. Actualice al SDK web de Experience Platform (preferido) o a la última versión de at.js.
...
Los siguientes recursos contienen información detallada para ayudarle a implementar la variable [!DNL Experience Platform Web SDK] o at.js:

* [[!DNL Adobe Experience Platform Web SDK] Extensión](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/sdk/overview.html?lang=es)
...
[!UICONTROL La segmentación automática utiliza aprendizaje automático avanzado para seleccionar entre varias experiencias de alto nivel de rendimiento definidas por expertos en marketing. ] [!UICONTROL La segmentación automática proporciona la experiencia más adaptada a cada visitante. ] La entrega de experiencias se basa en perfiles de clientes individuales y en el comportamiento de visitantes anteriores con perfiles similares. Uso [!UICONTROL Segmentación automática] para personalizar el contenido y dirigir las conversiones.
...
[!UICONTROL Multivariate Testing] (MVT) compara combinaciones de ofertas de elementos en una página para determinar qué combinación ofrece el mejor rendimiento para una audiencia específica. MVT ayuda a identificar qué elemento tiene el mayor impacto en el éxito de la actividad.
...
Bases de datos de actividades y contenido, [!DNL Analytics] los datos, las API y las interfaces de usuario de los especialistas en marketing se hospedan en los clústeres centrales de Adobe. Las actualizaciones se envían a los Edges de [!DNL Target]. Los clústeres centrales y de Edge se sincronizan automáticamente para actualizar de forma continua los datos de actividades en caché. Todo el modelado 1:1 también se almacena en cada Edge, de modo que las solicitudes más complejas también pueden ser procesadas en el Edge.
...
>[!IMPORTANT]
>
>[!DNL Adobe Target] actualmente no tiene un clúster de Edge en China y el rendimiento del visitante sigue siendo limitado para [!DNL Target] clientes en China. Debido al firewall y a la falta de clústeres de Edge dentro del país, las experiencias de los sitios con [!DNL Target] implementado pueden verse afectadas. Las experiencias pueden procesarse lentamente y las cargas de página pueden verse afectadas. Además, los especialistas en marketing pueden experimentar latencia al utilizar la IU de creación de [!DNL Target].
...
[!DNL Adobe] garantiza que la disponibilidad y el rendimiento de la infraestructura de segmentación sea lo más segura posible. Sin embargo, un desglose de comunicaciones entre el explorador de un visitante y [!DNL Adobe] los servidores de pueden provocar una interrupción en la entrega de contenido.
...
No se realizan cambios en la página si el explorador del usuario no se puede conectar dentro de un período de tiempo de espera definido (de forma predeterminada: 15 segundos). Si se alcanza este umbral de tiempo de espera, se muestra el contenido de ubicación predeterminado.
...
Google promueve las pruebas de usuario. Google indica en su documentación que A/B y [!UICONTROL Multivariate Testing] no perjudica la clasificación de los motores de búsqueda orgánicos si sigue determinadas directrices.
...
* Buscar [!DNL Adobe Audience Manager] (AAM) segmentos (si procede)
* Utilice el tráfico de bots para modelar y ofrecer contenido personalizado para [!UICONTROL Recommendations], [!UICONTROL Segmentación automática], [!UICONTROL Automated Personalization]o [!UICONTROL Asignación automática] actividades
...
Para el tráfico de bots conocido al usar [!UICONTROL Analytics para Target] (A4T), [!DNL Target] no:

* Enviar eventos a [!DNL Analytics]

Para el tráfico de bots conocido al utilizar el registro del lado del cliente, [!DNL Target] no devuelve:
