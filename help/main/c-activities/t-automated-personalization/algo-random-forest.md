...
description: Descubra cómo [!DNL Adobe Target] utiliza el algoritmo de bosque aleatorio en ambos [!UICONTROL Automated Personalization] (AP) y [!UICONTROL Segmentación automática] actividades.
...
Algoritmo de personalización principal utilizado en (AP) y [!DNL Auto-Target] actividades es bosque aleatorio. Los métodos de ensamblado, como el bosque aleatorio, utilizan varios algoritmos de aprendizaje para obtener un mejor rendimiento predictivo que el que se podría obtener de cualquiera de los algoritmos de aprendizaje constituyentes. Algoritmo de bosque aleatorio en [!UICONTROL Automated Personalization] y [!UICONTROL Segmentación automática] es un método de clasificación o regresión que opera mediante la construcción de una multitud de árboles de decisión cuando se está entrenando.
...
El algoritmo de bosque aleatorio es el algoritmo de personalización subyacente clave que se utiliza en [!UICONTROL Automated Personalization] y [!UICONTROL Segmentación automática] actividades. El bosque aleatorio combina cientos de árboles de decisiones para llegar a una mejor predicción de lo que un solo árbol podría hacer por sí mismo.
...
## ¿Cómo funcionan las [!DNL Target] ¿Los algoritmos de personalización utilizan el bosque aleatorio? {#section_32FB53CAD8DF40FB9C0F1217FBDBB691}

### Cómo se crean los modelos

El diagrama siguiente resume cómo se crean los modelos para [!UICONTROL Segmentación automática] y [!UICONTROL Automated Personalization] actividades:
...
1. Después [!DNL Target] alcanza una masa crítica de datos, [!DNL Target] realiza ingeniería de funciones
1. [!DNL Target] crea modelos de bosque aleatorio para cada experiencia u oferta
1. [!DNL Target] comprueba si el modelo llega al umbral de la puntuación de calidad.
1. [!DNL Target] lleva el modelo a producción para personalizar el tráfico futuro.

[!DNL Target] utiliza datos que recopila automáticamente, así como datos personalizados proporcionados por usted, para crear sus algoritmos de personalización. Estos modelos predicen la mejor experiencia u oferta para mostrar a los visitantes. Por lo general, se crea un modelo por experiencia (si existe un [!UICONTROL Segmentación automática] actividad) o por oferta (si existe [!UICONTROL Automated Personalization] actividad). [!DNL Target] a continuación, muestra la experiencia u oferta que produce la métrica de éxito más alta prevista (por ejemplo, tasa de conversión). Estos modelos deben prepararse a partir de visitas ofrecidas aleatoriamente para que se puedan utilizar para la predicción. Como consecuencia, al iniciar una actividad, se muestran aleatoriamente experiencias u ofertas diferentes incluso a los visitantes que se encuentran en el grupo personalizado mientras los algoritmos de personalización no están listos.
...
Después [!DNL Target] tiene modelos de personalización creados para personalizar su tráfico, hay un claro equilibrio que enfrenta para futuros visitantes de su actividad. ¿Debe personalizar todo el tráfico en función del modelo actual o debe seguir aprendiendo de los nuevos visitantes ofreciéndoles ofertas aleatorias? Le interesa asegurarse de que el algoritmo de personalización siempre aprenda de las tendencias nuevas de sus visitantes, al mismo tiempo que personaliza la mayor parte del tráfico.

El bandido multi-brazo es cómo [!DNL Target] le ayuda a alcanzar este objetivo. El método multi-armed bandit garantiza que el modelo siempre &quot;gasta&quot; una pequeña fracción del tráfico para seguir aprendiendo a lo largo de la vida de la actividad y para evitar la sobreexplotación de las tendencias aprendidas anteriormente.
