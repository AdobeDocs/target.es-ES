...
description: Descubra cómo una actividad de [!UICONTROL Segmentación automática] en [!DNL Target] sirve la experiencia más adaptada a cada visitante en función de los perfiles de los clientes y el comportamiento de visitantes similares.
...
>
>* [!UICONTROL Analytics for Target] (A4T) admite actividades de [!UICONTROL Segmentación automática]. Para obtener más información, consulte [Compatibilidad de A4T con actividades de asignación automática y segmentación automática](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md).

## Caso de éxito real con segmentación automática {#success}

Un importante proveedor de retailer de ropa recientemente usó una actividad [!UICONTROL Segmentación automática] con diez experiencias basadas en categorías de productos (además de un control aleatorio) para ofrecer el contenido adecuado a cada visitante. Se eligió &quot;[!UICONTROL Agregar al carro]&quot; como métrica de optimización principal. Las experiencias segmentadas tuvieron un alza media del 29,09 %. Después de crear los modelos [!UICONTROL Segmentación automática], la actividad se estableció en experiencias personalizadas al 90 %.

En solo diez días, se lograron más de 1.700.000 dólares en alza.

Siga leyendo para aprender a usar [!UICONTROL la segmentación automática] a fin de aumentar el alza y los ingresos de su organización.

## Información general {#section_972257739A2648AFA7E7556B693079C9}

Mientras [crea una actividad A/B](/help/main/c-activities/t-test-ab/t-test-create-ab/test-create-ab.md) mediante el flujo de trabajo guiado de tres pasos, elige la opción **[!UICONTROL Segmentación automática para experiencias personalizadas]** en la página **[!UICONTROL Segmentación]** (paso 2).

![Opción de segmentación automática para experiencias personalizadas](/help/main/c-activities/assets/auto-target-ui-new.png)

La opción [!UICONTROL Segmentación automática] dentro del flujo de actividad A/B le permite aprovechar el aprendizaje automático para personalizar en un solo clic basándose en un conjunto de experiencias definidas por el especialista en marketing. [!UICONTROL Segmentación automática] está diseñada para ofrecer la máxima optimización, en comparación con las pruebas A/B tradicionales o [!UICONTROL Asignación automática], al determinar qué experiencia mostrar para cada visitante. A diferencia de una actividad A/B en la que el objetivo es encontrar un solo ganador, [!UICONTROL Segmentación automática] determina automáticamente la mejor experiencia para un visitante determinado. La mejor experiencia se basa en el perfil del visitante y otra información contextual para ofrecer una experiencia altamente personalizada.

De manera similar a [!UICONTROL Automated Personalization], [!UICONTROL Segmentación automática] usa un [algoritmo de bosque aleatorio](/help/main/c-activities/t-automated-personalization/algo-random-forest.md), un método puntero de ensamblado de la ciencia de datos, para determinar la mejor experiencia para mostrar a un visitante. Debido a que [!UICONTROL Segmentación automática] puede adaptarse a los cambios en el comportamiento de los visitantes, puede ejecutarse de manera perpetua para proporcionar un aumento. Este método a veces se denomina modo &quot;siempre-activo&quot;.

A diferencia de una actividad A/B en la que la asignación de experiencias para un visitante dado es fija, [!UICONTROL la segmentación automática] optimiza el objetivo comercial especificado en cada visita. Al igual que en [!UICONTROL Auto Personalization], [!UICONTROL Segmentación automática], de manera predeterminada, reserva parte del tráfico de la actividad como grupo de control para medir el alza. A los visitantes del grupo de control se les sirve una experiencia aleatoria en la actividad.

## Consideraciones

Hay algunas consideraciones importantes que se deben tener en cuenta al usar [!UICONTROL Segmentación automática]:

* No puede cambiar una actividad específica de [!UICONTROL Segmentación automática] a [!UICONTROL Automated Personalization], y viceversa.
* No puede cambiar de la asignación de tráfico [!UICONTROL Manual] (tradicional [!UICONTROL prueba A/B]) a la [!UICONTROL Segmentación automática], y viceversa después de que una actividad se guarde como borrador.
...
## Cómo funciona [!UICONTROL Segmentación automática] {#section_77240E2DEB7D4CD89F52BE0A85E20136}

Obtenga más información acerca de los datos y algoritmos subyacentes a [!UICONTROL Segmentación automática] y [!UICONTROL Automated Personalization] en los siguientes enlaces:

| Término | Detalles |
|--- |--- |
| [Algoritmo de bosque aleatorio](/help/main/c-activities/t-automated-personalization/algo-random-forest.md) | El algoritmo de personalización principal de [!DNL Target] que se usa en [!UICONTROL Segmentación automática] y [!UICONTROL Automated Personalization] es el de bosque aleatorio. Los métodos de ensamblado, como el bosque aleatorio, utilizan varios algoritmos de aprendizaje para obtener un mejor rendimiento predictivo que el que se podría obtener de cualquiera de los algoritmos de aprendizaje constituyentes. El algoritmo de bosque aleatorio en las actividades [!UICONTROL Automated Personalization] y [!UICONTROL Segmentación automática] es un método de clasificación o regresión que funciona mediante la construcción de una multitud de árboles de decisión en el momento de entrenar. |
| [Cargando datos para los algoritmos Personalization de [!DNL Target]](/help/main/c-activities/t-automated-personalization/algo-random-forest.md) | Hay varias formas de introducir datos para los modelos [!UICONTROL Segmentación automática] y [!UICONTROL Automated Personalization]. |
...
* [!UICONTROL Evaluar el algoritmo de Personalization]
* [!UICONTROL Maximizar el tráfico de Personalization]
* [!UICONTROL Asignación personalizada]
...
Para ajustar el porcentaje [!UICONTROL Control], haga clic en los iconos de la columna [!UICONTROL Asignación]. No se puede reducir el grupo de control por debajo del 10 %.
...
## ¿Cuándo debería elegir [!UICONTROL Segmentación automática] en lugar de [!UICONTROL Automated Personalization]? {#section_BBC4871C87944DD7A8B925811A30C633}

Hay varios escenarios en los que puede preferir usar [!UICONTROL Segmentación automática] sobre [!UICONTROL Automated Personalization]:
...
* Si desea usar el conjunto completo de características de [!UICONTROL Compositor de experiencias visuales] (VEC) que no admite [!UICONTROL Auto Personalization]: el editor de código personalizado, las audiencias de experiencias múltiples y más.
* Si desea realizar cambios estructurales en su página en diferentes experiencias. Por ejemplo, si deseas reorganizar elementos en tu página de inicio, [!UICONTROL la segmentación automática] es más apropiada para usar que [!UICONTROL Automated Personalization].

## ¿Qué tienen en común [!UICONTROL Segmentación automática] y [!UICONTROL Automated Personalization]? {#section_2A601F482F9A44E38D4B694668711319}
...
* Esta métrica puede estar basada en conversiones o en ingresos (más específicamente [!UICONTROL Ingresos por visita]).

### [!DNL Target] recopila automáticamente información sobre los visitantes para crear los modelos de personalización.

* Para obtener más información sobre los parámetros utilizados en [!UICONTROL Segmentación automática] y [!UICONTROL Automated Personalization], consulte [Recopilación de datos de Automated Personalization](/help/main/c-activities/t-automated-personalization/ap-data.md).
...
* Más información sobre [cargar datos para [!UICONTROL Segmentación automática] y [!UICONTROL Automated Personalization]](/help/main/c-activities/t-automated-personalization/uploading-data-for-the-target-personalization-algorithms.md).

## ¿En qué se diferencian [!UICONTROL Segmentación automática] y [!UICONTROL Automated Personalization]? {#section_BA4D83BE40F14A96BE7CBC7C7CF2A8FB}

### [!UICONTROL La segmentación automática] requiere con frecuencia menos tráfico que [!UICONTROL Automated Personalization] para crear un modelo personalizado.

Aunque la cantidad de tráfico *por experiencia* requerido para que los modelos de [!UICONTROL Segmentación automática] o [!UICONTROL Personalization automático] se creen es la misma, usualmente hay más experiencias en una actividad de [!UICONTROL Automated Personalization] que una actividad de [!UICONTROL Segmentación automática].

Por ejemplo, si tuvo una actividad [!UICONTROL Auto Personalization] en la que creó dos ofertas por ubicación con dos ubicaciones, habría cuatro (2 = 4) experiencias totales incluidas en la actividad (sin exclusiones). Con [!UICONTROL Segmentación automática], podría establecer la experiencia 1 para incluir la oferta 1 en la ubicación 1 y la oferta 2 en la ubicación 2, y la experiencia 2 para incluir la oferta 1 en la ubicación 1 y la oferta 2 en la ubicación 2. Debido a que [!UICONTROL Segmentación automática] le permite elegir tener múltiples cambios dentro de una experiencia, puede reducir la cantidad total de experiencias en su actividad.

Para [!UICONTROL Segmentación automática], se pueden usar reglas simples para comprender los requisitos de tráfico:

* **Cuando [!UICONTROL Conversión] es tu métrica de éxito:** 1000 visitas y al menos 50 conversiones por día y experiencia, además la actividad debe tener al menos 7000 visitas y 350 conversiones.
* **Cuando [!UICONTROL Ingresos por visita] es su métrica de éxito:** 1000 visitas y al menos 50 conversiones por día y experiencia, además la actividad debe tener al menos 1000 conversiones por experiencia. Por lo general, el RPV requiere más datos para construir modelos debido a la mayor varianza de datos que normalmente existe en los ingresos de visita en comparación con la tasa de conversión.

### [!UICONTROL Segmentación automática] tiene una funcionalidad de configuración completa.

* Dado que [!UICONTROL Segmentación automática] está integrada en el flujo de trabajo de la actividad A/B, [!UICONTROL Segmentación automática] se beneficia del [!UICONTROL Compositor de experiencias visuales] (VEC), que es un producto más maduro y completo. También puede usar [vínculos de control de calidad](/help/main/c-activities/c-activity-qa/activity-qa.md) con [!UICONTROL Segmentación automática].

### [!UICONTROL Segmentación automática] proporciona un amplio marco de pruebas en línea.
...
En este vídeo se explica cómo configurar una actividad A/B de [!UICONTROL Segmentación automática].

Tras completar esta formación, debe ser capaz de:

* Definir las pruebas de [!UICONTROL Segmentación automática]
* Comparar y comparar [!UICONTROL Segmentación automática] con [!UICONTROL Automated Personalization]
* Crear [!UICONTROL actividades de Segmentación automática]
