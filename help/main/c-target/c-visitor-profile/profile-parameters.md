...
| `user.get('<par_name>')` | Devuelve el valor de la variable &quot;`user.<par_name>`&quot; configurado para el visitante actual; devuelve nulo si no se ha establecido ningún valor. |
...
| `>=` | Igual que `>`, excepto si las variables son iguales, que entonces se evaluará como true. |
| `&&` | Lógicamente &quot;AND&quot; las expresiones a la izquierda y a la derecha de la misma, son solo verdaderas cuando ambos lados son verdaderos (falso en caso contrario). |
