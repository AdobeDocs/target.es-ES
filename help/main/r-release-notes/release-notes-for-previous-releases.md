...
* Se perdió **código personalizado al aplicarse en vistas adicionales.** Esta corrección resuelve un problema en el cual el código personalizado aplicado a una vista se perdía al agregar y aplicar código personalizado a una vista adicional. Anteriormente, cuando los usuarios aplicaban código personalizado a varias vistas en una actividad, el código personalizado aplicado anteriormente desaparecía. La corrección garantiza que el código personalizado se mantenga correctamente en todas las vistas al aplicar modificaciones a vistas adicionales. (TGT-54230)
...
* **Cambiar la posición de la página de perspectivas después de todas las actividades.**: la posición de la **página de perspectivas** se ha actualizado para que aparezca después de la página de **Todas las actividades** en la navegación. La página Todas las actividades es ahora la página predeterminada de la aplicación de Target. (TGT-54672)
...
* **ExperienceEditorType predeterminado en &quot;visual&quot; cuando el backend devuelve nulo.** Esta corrección soluciona un problema en el que el valor `experienceEditorType` se devolvía como nulo en algunos casos, lo que provocaba un comportamiento no deseado en la interfaz de usuario en el que el tipo de editor no se inicializaba correctamente. (TGT-54690)
...
### [!DNL Target Standard/Premium] 26.1.2 (sábado, 30 de enero de 2026)
...
* **Habilite la columna del área de trabajo que ofrece el editor web.** Esta actualización soluciona la confusión del cliente causada por ofertas de [!UICONTROL Default Workspace] que aparecen en otros espacios de trabajo del Editor web. Aunque este comportamiento funciona correctamente, las ofertas de [!UICONTROL Default Workspace] son visibles intencionadamente en todos los espacios de trabajo, los clientes informaron de que la interfaz de usuario no aclaraba el origen del espacio de trabajo, especialmente al crear actividades en un espacio de trabajo no predeterminado como &quot;Aprobadores&quot;. Para mejorar la claridad, la columna [!UICONTROL Workspace] se ha habilitado ahora en la lista de ofertas del Editor Web, lo que permite a los usuarios distinguir fácilmente a qué área de trabajo pertenece cada oferta y evita una interpretación errónea de las ofertas adicionales mostradas. (TGT-54138)
* Los **vínculos con target=&quot;_blank&quot; se abren en una nueva pestaña.** Esta corrección soluciona un problema por el que los sitios web creados que contienen vínculos con ~target=&quot;_blank&quot;~ se abrían en una nueva pestaña del explorador al hacer clic en ella en modo [!UICONTROL Browse], lo que interrumpe la experiencia de vista previa en el editor. El comportamiento se producía porque los atributos de vínculo nativo de la página creada no eran interceptados por el JavaScript insertado de la extensión, a diferencia de la interfaz de usuario heredada, donde los elementos de anclaje se transformaban y sus destinos se reemplazaban para mantener la navegación dentro del editor. La actualización garantiza que los vínculos que usan ~target=&quot;_blank&quot;~ ahora se gestionen correctamente dentro del Editor web, de modo que ya no abran pestañas externas durante la creación. (TGT-54134)
...
* **Se ha corregido un problema en el que la flecha [!UICONTROL Reports] no indicaba claramente las columnas expandibles.** Solucionó un problema en el que la tabla de informes no mostraba claramente que se podían expandir columnas adicionales en la interfaz de usuario actualizada. Se ha agregado un consejo de herramientas que desaparece a la flecha [!UICONTROL Reports] cerca de los encabezados de columna para ayudar a los clientes a comprender que hay más columnas disponibles. (TGT-53479)
...
### [!DNL Target Standard/Premium] 25.10.1 (jueves, 22 de octubre de 2025)
...
* El modo **[!UICONTROL Design]en VEC ahora bloquea la navegación de vínculos para componentes editables.** Se ha resuelto un problema en la VEC actualizada por el que los elementos en los que se puede hacer clic, como botones y vínculos, desencadenaban la redirección de páginas incluso en el modo [!UICONTROL Design]. Este comportamiento imitaba el modo [!UICONTROL Browse] e impedía que los usuarios modificaran componentes clave. (TGT-53696)
* La métrica **&quot;[!UICONTROL Clicked an element]&quot; ahora funciona sin redirección ni error.** Se ha resuelto un problema en la nueva interfaz de usuario de creación que provocaba redirecciones inesperadas y pantallas en blanco al seleccionar la métrica de conversión &quot;[!UICONTROL Clicked an element]&quot;. Al hacer clic en un botón durante la instalación, se activó la navegación en lugar de registrar el elemento, lo que produjo el error &quot;[!UICONTROL element not found]&quot;. (TGT-53817)
...
* **Las páginas de actividad existentes con modificaciones ahora se cargan correctamente en el VEC.** Resolvió un problema en la VEC actualizada que hizo que las páginas permanecieran atascadas en la fase de carga al editar una actividad existente con las modificaciones guardadas. Las nuevas actividades se cargaron sin problemas, pero las actividades configuradas anteriormente no se procesaron. (TGT-53967)

+++

### [!DNL Target Standard/Premium] 25.9.3 (miércoles, 30 de septiembre de 2025)

Esta versión contiene las siguientes mejoras y correcciones.

+++[!UICONTROL Audiences]

* **Las reglas de exclusión de audiencia se mostraron incorrectamente como inclusión en la interfaz de usuario de [!DNL Target].** Las audiencias configuradas con reglas de exclusión aparecían como incluidas al editar destinos dentro de una actividad. Aunque la lógica de exclusión se aplicó correctamente durante la ejecución, la interfaz de usuario no reflejó la regla correctamente, omitiendo la etiqueta &quot;excluyendo&quot;. La interfaz de usuario de [!DNL Target] ahora muestra correctamente las reglas de exclusión tanto en la configuración de audiencia como en los flujos de trabajo de destinatarios, lo que garantiza la claridad y la coherencia en la configuración de la campaña. (TGT-53808)
* **La sección [!UICONTROL Targeting] no indica que se haya establecido una regla de audiencia para excluirla.** Las audiencias configuradas con lógica de exclusión se mostraron incorrectamente como inclusión en la sección [!UICONTROL Targeting] de la interfaz de usuario de creación de actividad. Aunque el servidor aplicó correctamente la regla de exclusión, la interfaz de usuario no la representó visualmente, omitiendo la etiqueta &quot;Excluir&quot; y causando confusión durante la configuración de la campaña. La sección [!UICONTROL Targeting] muestra ahora claramente las reglas de exclusión, lo que garantiza la coherencia entre la configuración de audiencia y la visualización de destinatarios. (TGT-53809)
...
### [!DNL Target Standard/Premium] 25.9.2 (martes, 22 de septiembre de 2025)
...
### [!DNL Target Standard/Premium] 25.9.1 (sábado, 05 de septiembre de 2025)
...
* **Se ha corregido la lógica del formulario para las reglas de presencia de parámetros en la entrega de páginas.&quot;** ha resuelto un problema en la interfaz de usuario actualizada que hacía que, al seleccionar reglas de entrega de páginas como &quot;[!UICONTROL Parameter is present]&quot;, &quot;[!UICONTROL Parameter is not present]&quot;, &quot;[!UICONTROL Parameter value is present]&quot; o &quot;[!UICONTROL Parameter value is not present]&quot;, se exigiera incorrectamente a los usuarios que especificaran un valor de parámetro adicional. Este comportamiento fue inconsistente con la interfaz de usuario heredada y contradijo la lógica prevista de detectar la presencia de parámetros sin especificar un valor. Esta corrección restaura el comportamiento de configuración de reglas esperado, optimizando la configuración de actividades y mejorando la usabilidad. (TGT-53640)
* **Se ha mejorado la lógica de validación para el generador de reglas de varias páginas en la interfaz de usuario actualizada.** Resolvió varios problemas de validación en el generador de reglas de varias páginas dentro de la interfaz de usuario actualizada. Estas correcciones incluyen:
...
### [!DNL Target Standard/Premium] 25.8.4 (martes, 01 de septiembre de 2025)
...
* **Ingesta proactiva de datos de clientes [!DNL Target] depurados durante la creación de la actividad**: se ha mejorado el proceso de creación de actividades al habilitar la recopilación proactiva de informes, contenido y capturas de pantalla de [!DNL Target] clientes. Esta mejora soluciona las lagunas de datos identificadas en los casos de uso existentes y ayuda a garantizar información más precisa durante la configuración de la actividad y el experimento. (TGT-52415)
...
+++Ver detalles
* **La lista de productos no estaba visible en el diálogo [!UICONTROL View Collection]:** Anteriormente, los clientes no podían ver la lista de productos al ver una colección en la ficha [!UICONTROL Recommendations]. El diálogo [!UICONTROL View Collection] ahora muestra correctamente los productos asociados, mejorando la transparencia y la usabilidad en la interfaz de usuario [!UICONTROL Recommendations] actualizada. (TGT-50531)
* **Se ha corregido un problema que provocó el filtrado de mayúsculas y minúsculas en la búsqueda avanzada [!UICONTROL Product Catalog Search]**: el filtrado de búsqueda avanzado en la página [!UICONTROL Product Catalog Search] ignora correctamente la sensibilidad de mayúsculas y minúsculas, alineándose con el comportamiento de los servicios backend y GraphQL. Esta actualización garantiza resultados de sugerencias coherentes y precisos para los clientes, independientemente del formato de texto. (TGT-53585)
...
### [!DNL Target Standard/Premium] 25.8.3 (viernes, 21 de agosto de 2025)
...
### [!DNL Target Standard/Premium] 25.8.2 (viernes, 14 de agosto de 2025)
...
**Fragmentos de experiencia (XF)**

+++Ver detalles
* **Se ha corregido un problema en el proceso de creación de actividad que permitía la edición no intencionada por parte del HTML de fragmentos AEM-exportados**: Se ha corregido un problema en el proceso de creación de actividad que permitía a los clientes editar el HTML de [!DNL Experience Fragments] (XF) exportado desde [!DNL Adobe Experience Manager] (AEM) en [!DNL Target]. Este comportamiento no fue intencionado, ya que los archivos XF deberían permanecer bloqueados para su edición una vez publicados desde AEM. La corrección garantiza que la opción &quot;Editar HTML&quot; ya no esté disponible para los fragmentos exportados AEM, lo que conserva la integridad del contenido y el control previsto. (TGT-53286)
...
+++Ver detalles
* **Se ha corregido un problema en el proceso de creación de actividad en el que los espacios principales de las direcciones URL causaban vínculos QA rotos**: Se ha corregido un problema en el proceso de creación de actividades en el que los espacios principales de la dirección URL de la actividad no se habían recortado correctamente al guardar. Esto provocaba vínculos de control de calidad no válidos y un formato incorrecto en el back-end. Después de la actualización, las direcciones URL ahora se guardan de forma limpia, lo que evita que se rompan los vínculos y mejora la fiabilidad de los flujos de trabajo de control de calidad para los clientes. (TGT-53427)
...
+++Ver detalles
* **Se ha corregido un problema en la interfaz de usuario de búsqueda de catálogos en el que la búsqueda avanzada no pudo proporcionar sugerencias**: Se ha corregido un problema en la nueva interfaz de usuario de [!UICONTROL Catalog Search] en el que la característica [!UICONTROL Advanced Search] no pudo proporcionar sugerencias. Se pidió a los usuarios que introdujeran valores exactos con una ortografía precisa, lo que dificultó la búsqueda y provocó errores. Con esta corrección, [!UICONTROL Advanced Search] ahora ofrece sugerencias relevantes a medida que los usuarios escriben, lo que mejora la facilidad de uso y reduce la fricción al localizar productos. (TGT-52008)
* **Se han resuelto varios problemas de filtrado e interfaz de usuario para mejorar la capacidad de respuesta y la interacción con la entidad**: se han resuelto varios problemas, incluida la respuesta deficiente de los detalles de criterios en dispositivos de pantalla pequeña, la falta de resultados al seleccionar &quot;Todos los grupos de hosts&quot; en el filtro Entorno y la incapacidad para interactuar con entidades que no tienen nombre. Estas correcciones mejoran la facilidad de uso en todos los tamaños de pantalla, garantizan un filtrado preciso y permiten una interacción completa con todas las entidades de producto, lo que mejora la experiencia general de los usuarios. (TGT-52992)
...
* **Se ha corregido un problema en el VEC por el cual los cambios en el código personalizado no se guardaban de forma fiable entre sesiones de edición**: se ha corregido un problema por el que las modificaciones de código personalizado realizadas en el Compositor de experiencias visuales (VEC) no se guardaban de forma fiable en un solo intento. Los clientes informaron de que los cambios, como las actualizaciones de estilo o la edición HTML, faltaban intermitentemente en el sitio web y en las direcciones URL de QA, incluso después de utilizar los botones [!UICONTROL Edit Content] y [!UICONTROL Save] final. Esta regresión se ha resuelto para garantizar que todos los cambios en el código personalizado persistan según lo esperado en las sesiones de edición.** (TGT-53418)
...
### [!DNL Target Standard/Premium] 25.8.1 (viernes, 07 de agosto de 2025)
...
### [!DNL Target Standard/Premium] 25.7.4 (sábado, 01 de agosto de 2025)
...
+++Ver detalles
* Se ha corregido un problema por el que la invocación de `triggerView` mediante código personalizado provocaba un bucle infinito. (TGT-52885)
* Se ha corregido un problema que causaba discrepancias entre `LocalIds` definido para actividades y los `LocalIds` utilizados en definiciones de experiencia. (TGT-52669)
...
+++Ver detalles
* Se ha corregido un problema en el que al hacer clic en un producto de [!UICONTROL Catalog Search] se muestra el error &#39;Error al recuperar los detalles del producto&#39; y se abre un modal sin una opción de cierre. (TGT-53082)
...
+++Ver detalles
* Se ha corregido un problema en el que al guardar los detalles del pedido en un archivo CSV se ha producido un archivo vacío. (TGT-52225)
...
* Se ha corregido un problema que impedía la edición de la actividad y que desencadenaba el mensaje de error: &quot;No se pueden leer las propiedades de undefined (reading &#39;map&#39;)&quot;. (TGT-53161)
...
* Se ha corregido un problema en el que la característica [!UICONTROL Manage Content] no validó correctamente el estado de una ubicación después de que se eliminaran todas las opciones de contenido. Este problema podría dar lugar a comportamientos incoherentes o errores al intentar guardar o continuar con la configuración de la actividad. (TGT-52801)
* Se corrigió un problema en el que los usuarios encontraron un error de &quot;entrada no válida&quot; al agregar una página nueva y eliminar elementos específicos dentro de experiencias diferentes. El error se desencadena cuando se genera un duplicado de `LocalIds` durante la manipulación de elementos, especialmente al cambiar entre experiencias y modificar estructuras de páginas compartidas. (TGT-52720)
* Se ha corregido un problema por el que el uso de la característica [!UICONTROL Generate Adhoc Offer] provocaba que aparecieran ubicaciones no definidas en el panel [!UICONTROL Manage Content]. (TGT-53076 y TGT-53070)
* Se ha aclarado el comportamiento del cliente en el que las modificaciones realizadas mediante una oferta de HTML podrían parecer no aparecer al navegar desde el paso anterior de [!UICONTROL Targeting] hasta [!UICONTROL Experiences]. Para este cliente, el sitio web afectado generaba dinámicamente varios selectores DOM que cambiaban con cada carga de página. Como resultado, el selector utilizado originalmente para la modificación no se puede encontrar cuando se vuelve a abrir el editor, lo que hace que la modificación parezca no estar presente o no ser válida. Este escenario funciona según lo previsto. Para garantizar que las modificaciones persistan visualmente en el editor, se recomienda que los clientes utilicen selectores estables y consistentes que no cambien de página a página. (TGT-52874)
...
* Se eliminó la validación innecesaria al seleccionar el espacio de trabajo predeterminado durante la creación de la actividad. La validación obligatoria de propiedades ya no se aplica al espacio de trabajo predeterminado, pero permanece vigente para espacios de trabajo no predeterminados. (TGT-52449)
...
* Se ha corregido un problema en el cual la interfaz de usuario no permitía la asignación manual de un ID de entorno a un criterio. En su lugar, se estableció de forma predeterminada el Id. del grupo host [!UICONTROL Product Catalog Search]. Esta corrección garantiza que los cambios de criterios se apliquen ahora a todos los entornos, no solo al predeterminado. (TGT-52817)
...
### [!DNL Target Standard/Premium] 25.4.5 (sábado, 25 de abril de 2025)
...
* Se ha corregido un error que impedía la edición de una actividad XT debido a una regla de interfaz de usuario no compatible. (TGT-52273)
* Se ha corregido un problema en la actualización de [!UICONTROL Visual Experience Composer] (VEC) por el que no siempre se mostraban las rutas de exploración en la parte inferior del editor, lo que provocaba dificultades en la selección precisa de los elementos. (TGT-52169)
* Se ha corregido un problema por el que la lista desplegable [!UICONTROL Audience] no podía mostrar todas las audiencias debido a la paginación. (TGT-52204)
* Se ha corregido un problema que provocaba un mensaje de entrada de usuario no válido al agregar nuevas ofertas en actividades de [!UICONTROL Automated Personalization] (PA). (TGT-52210)
...
* Se ha corregido un problema que impedía a los clientes seleccionar espacios de trabajo mientras creaban ofertas dentro de una actividad. (TGT-52289)
...
* Se ha corregido un problema por el que las modificaciones de una experiencia se mostraban incorrectamente al cambiar a otra experiencia. (TGT-52184)
* Se ha corregido un problema por el que la oferta predeterminada se mostraba incorrectamente en la interfaz de usuario de [!DNL Target] al abrir la actividad. (TGT-52198)

### Actualización de permisos de destino (22 de abril de 2025)

Esta actualización futura mejora el control organizativo sobre las configuraciones de instancias de [!DNL Target], lo que evita actualizaciones accidentales que podrían afectar a la entrega de actividades en varios equipos de pruebas y personalización.
...
### [!DNL Target Standard/Premium] 25.4.4 (viernes, 17 de abril de 2025)
...
* `OptionLocalIDs` ya no se incrementa incorrectamente cuando la opción no cambia. (TGT-52139)
* Se ha corregido un problema que provocaba un mensaje &quot;No válido `optionLocalIds`&quot; al intentar crear una actividad. (TGT-52154)
...
### [!DNL Target Standard/Premium] 25.4.3 (sábado, 11 de abril de 2025)
...
### [!DNL Target Standard/Premium] 25.4.1 (jueves, 02 de abril de 2025)
...
* Se ha corregido un problema que bloqueaba la edición de ofertas ad hoc creadas en la interfaz de usuario [!DNL Target] heredada. (TGT-51984)
* Se ha corregido un problema que bloqueaba las actividades de edición que tenían ofertas ad hoc que contenían código personalizado. (TGT-51995)
* Se corrigió un problema que hizo que las reglas de exclusión se mostraran como reglas de inclusión al editar definiciones de audiencia combinadas. (TGT-51999)
...
* Se corrigió un problema que desencadenó un mensaje `default message [Invalid optionLocalIds: xx]]` al crear una actividad. (TGT-52038)
...
* [Extensión  [!DNL Chrome] actualizada](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md) compatible con Manifest V3 para aumentar la seguridad y mejorar la compatibilidad con cookies de primera parte.
...
Con el último sistema de diseño de [!DNL Adobe Spectrum], esta actualización estandariza los patrones de diseño incoherentes e introduce nuevas mejoras, incluidas las siguientes:
...
![Ofrece vídeo de actualización de la interfaz de usuario](/help/main/r-release-notes/assets/offers-video-v2.gif)
...
>[!IMPORTANT]
>
>Si no puede completar la revisión de implementación y resolver ningún problema antes del 3 de febrero de 2025, puede solicitar una extensión única de seis meses. Asegúrese de que su solicitud se envía antes del 31 de enero de 2025. Adobe revisará y decidirá su solicitud.

Para beneficiarse de esta optimización en caso de representación manual, revise su [[!DNL Platform Web SDK implementation]](https://experienceleague.adobe.com/en/docs/target-dev/developer/client-side/aep-web-sdk){target=_blank} para asegurarse de que está enviando notificaciones después de representar manualmente experiencias o cuando utiliza el método `applyPropositions` (o la acción [!DNL Launch] correspondiente como auxiliar) para representar experiencias.

Los escenarios más comunes cuando las experiencias se representan manualmente incluyen:

* Uso de ofertas JSON
* Utilizar un ámbito de decisión personalizado en una actividad creada en [[!UICONTROL Form-Based Experience Composer]](/help/main/c-experiences/form-experience-composer.md)
* No se usa `renderDecisions: true` al obtener una actividad creada con [!UICONTROL Form-Based Experience Composer] que usa el ámbito global `__view__`

Si las notificaciones no se implementan como se documenta en [Procesar contenido personalizado](https://experienceleague.adobe.com/en/docs/experience-platform/web-sdk/personalization/rendering-personalization-content){target=_blank} en la guía de *Recopilación de datos*, es posible que falten datos de informes en [!DNL Target] y en [Análisis de informes de destino](/help/main/c-integrating-target-with-mac/a4t/a4t.md) (A4T). En ciertos casos, es posible que observe una división de tráfico incorrecta porque no se capturan los datos de informes. O, en otros casos, informar del mismo evento repetidamente.

Dependiendo de su implementación, compruebe los impactos de informe de [!DNL Analytics] y A4T.
...
* Se ha corregido un problema que hizo que el informe [!UICONTROL Important Attributes] no se mostrara para un solo cliente. (TGT-47272)
...
* Se ha corregido un problema que causó que los informes de [!UICONTROL Auto Personalization] actividades se congelaran durante el análisis. (TGT-44820)
* Se ha corregido un problema que impidió buscar una actividad en la página [!UICONTROL Activity] si el nombre de la actividad contiene un corchete ( [o] ). (TGT-44777)
...
* Se ha corregido un problema que impedía a un cliente editar refinamientos y audiencias de nivel de experiencia para una actividad si se creaban refinamientos y audiencias antes de que se implementara la nueva interfaz de usuario [!UICONTROL Audiences]. (TGT-43433)
...
* Se ha corregido un problema que afectaba a los informes de [!DNL Adobe Target Premium] de los usuarios de [!UICONTROL Auto-Target] desde el 15 de septiembre a las 2 :30 p. m. (PDT) hasta el 6 de octubre a las 9:25 a. m. (PDT). Cuando se visualizan informes para las métricas de conversión afectadas (configuradas mediante la opción &quot;[!UICONTROL Viewed a page]&quot; o &quot;[!UICONTROL Clicked on mbox]&quot;), las tasas de conversión se informan incorrectamente. No hay ningún problema de entrega conocido en este momento.
...
* Hemos estandarizado la lista de métricas disponibles de las actividades de Adobe Analytics para Target (A4T) al desaprobar las métricas de Adobe Analytics no admitidas en la versión actual de las API de Adobe Analytics. Esto nos permitirá ampliar nuestra compatibilidad con A4T en futuras versiones de Adobe Target.
...
| Función/Mejora | Descripción |
|---|---|
| Permitir inserción y edición de JavaScript | Se ha añadido la capacidad de editar e insertar JavaScript personalizado en el editor de experiencia al elegir **[!UICONTROL Edit HTML]** en el menú de acciones. |
