...
Esta página lista los cambios importantes realizados en la [!DNL Adobe Target] documentación del producto.

## Adobe Target Standard/Premium 21.1.1 (19 de enero de 2021)

| Fecha | Tema | Cambios |
| --- | --- | --- |
| Enero de 26 | [Tasa de conversión](/help/c-reports/conversion-rate.md#t-test) | Se ha aclarado cómo el Destinatario utiliza la &quot;suma de cuadrados&quot; en las pruebas T del estudiante. |
...
| Enero de 19 | [Notas de la versión de Target (actual)](/help/r-release-notes/release-notes.md) | Información añadida sobre la versión 21.1.1 de Destinatario (19 de enero de 2021). |
|  | [Límites](/help/r-troubleshooting-target/target-limits.md) | Se ha actualizado el texto del parámetro `productPurchasedID`. |
|  | [Problemas conocidos y problemas resueltos](/help/r-release-notes/known-issues-resolved-issues.md) | Se ha añadido un problema conocido al copiar una actividad [!UICONTROL de recomendación] con una promoción activa. Cualquier cambio en la actividad de duplicado también afecta a la actividad original y viceversa. Se incluye una solución temporal. |
...
| 27 de octubre | [Servidor: implementación de Target](/help/c-implementing-target/c-api-and-sdk-overview/api-and-sdk-overview.md) | Vínculo añadido al nuevo portal *[SDK de Adobe Target](https://adobetarget-sdks.gitbook.io/docs/)*. |
|  | [Crear una actividad que utilice Analytics como fuente de sistema de informes](/help/c-integrating-target-with-mac/a4t/campaign-creation.md) | Se ha añadido información que indica que si está utilizando `analyticsLogging = client_side`, debe pasar el valor `sessionId` a [!DNL Analytics] al usar Analytics como origen de sistema de informes (A4T) con actividades de Destinatario automático. |
|  | [Implementación de Analytics for Target](/help/c-integrating-target-with-mac/a4t/a4timplementation.md) | Información añadida que indica que para actividades [!UICONTROL de asignación automática] y [!UICONTROL de Destinatario automático] que utilizan `analyticsLogging = client_side`, también deberá reenviar el sessionId. |
...
| 22 de octubre | [CNAME y Adobe Target](/help/c-implementing-target/c-considerations-before-you-implement-target/implement-cname-support-in-target.md) | Información añadida sobre una corrección de la compatibilidad con CNAME en las versiones 1.8.2 y 2.3.1 de at.js |
|  | [targetGlobalSettings](/help/c-implementing-target/c-implementing-target-for-client-side-web/targetgobalsettings.md) | Información añadida sobre una corrección de la compatibilidad con CNAME en las versiones 1.8.2 y 2.3.1 de at.js |
...
| 12 de octubre | [Resolución de problemas de la entrega de contenido](/help/c-activities/c-troubleshooting-activities/content-trouble.md) | Se ha actualizado [Recupere el token de autorización para utilizarlo con las herramientas de depuración](/help/c-activities/c-troubleshooting-activities/content-trouble.md#section_BED130298E794D1FA229DB7C3358BA54) para indicar el nivel de permiso que debe tener para generar un token de autenticación. |
...
|  | [Segmentación automática](/help/c-activities/auto-target/auto-target-to-optimize.md) | Se añadió la historia de éxito del mundo real mediante el Destinatario automático. |
...
| 23 de septiembre | [Uso de un servidor de seguimiento de Analytics](/help/c-integrating-target-with-mac/a4t/analytics-tracking-server.md) | Se ha actualizado todo el tema con información de las [!DNL Adobe Experience Platform Debugger] y las herramientas para desarrolladores del explorador. |
...
|  | [targetGlobalSettings()](/help/c-implementing-target/c-implementing-target-for-client-side-web/targetgobalsettings.md) | Texto añadido que indica que si utiliza `serverState` y `<script>` etiquetas en el contenido devuelto, asegúrese de que el contenido HTML utilice `<\/script>` en lugar de `</script>`. |
...
| 6 de agosto | [Segmentación automática](/help/c-activities/auto-target/auto-target-to-optimize.md#how-long) | Se ha actualizado el texto de las siguientes preguntas más frecuentes: &quot;¿Cuánto tiempo debo esperar a que se construyan los modelos?&quot; |
...
| 3 de agosto | [Métricas de éxito](/help/c-activities/r-success-metrics/success-metrics.md) | Nota añadida que aclara el significado de las opciones [!UICONTROL Aumentar recuento] con respecto a visitantes vs. visitas. |
...
|  | [adobe.target.getOffers(options) - at.js 2.x](/help/c-implementing-target/c-implementing-target-for-client-side-web/adobe-target-getoffers-atjs-2.md) | Ejemplo de código añadido para utilizar `getoffers()` para realizar un pageLoad. |
...
|  | [Segmentación automática](/help/c-activities/auto-target/auto-target-to-optimize.md) | Se ha añadido la siguiente pregunta muy frecuente: <ul><li>¿Puedo utilizar la opción Restablecer datos del informe al ejecutar una actividad de Destinatario automático?</li></ul>Se ha actualizado el texto en la sección &quot;Consideraciones&quot;. |
...
|  | [Atributos del cliente](/help/c-target/c-visitor-profile/working-with-customer-attributes.md) | Se ha actualizado el siguiente párrafo:<br>[!DNL Adobe] no garantiza que el 100% de los datos de atributos del cliente (perfil de visitante) de las bases de datos CRM se incorporarán a [!DNL Experience Cloud] y, por lo tanto, estarán disponibles para su uso como objetivo en [!DNL Target]. En nuestro diseño actual, existe la posibilidad de que un pequeño porcentaje de datos (hasta el 0,1% de los lotes de producción grandes) no se incorporen. |
...
| Julio de 14 | [Preguntas más frecuentes sobre asignación](/help/c-activities/automated-traffic-allocation/automated-traffic-allocation.md) automática, Destinatario<br>[ ](/help/c-activities/auto-target/auto-target-to-optimize.md)automático <br><br>[y personalización automatizada](/help/c-activities/t-automated-personalization/automated-personalization-faq.md) | Preguntas más frecuentes añadidas que recomiendan que no debe cambiar la métrica de objetivos a mitad de una actividad. |
...
|  | [Información general sobre la documentación de destinatario](/help/r-release-notes/target-documentation.md) | Nuevo tema que detalla las diferentes fuentes de la documentación [!DNL Target]. |
...
|  | [Tokens de respuesta](/help/administrating-target/response-tokens.md) | Información añadida sobre tokens de respuesta para el método de asignación de tráfico para actividades [!UICONTROL Destinatario automático] y [!UICONTROL Automated Personalization]. |
...
|  | [Usuarios](/help/administrating-target/c-user-management/c-user-management/user-management.md) | Se añadió información sobre la nueva función [!UICONTROL Publicador] en *Especificar funciones y permisos*. |
|  | [Configuración de permisos de Enterprise](/help/administrating-target/c-user-management/property-channel/properties-overview.md) | Información añadida sobre la nueva función [!UICONTROL Publisher] en *Paso 6: Especifique roles y permisos*. |
|  | [Permisos de usuario de Enterprise](/help/administrating-target/c-user-management/property-channel/property-channel.md) | Vínculo añadido a las *horas de Office: Sesión de espacios de trabajo de destinatario Premium*. |
...
|  | [Notas de la versión de Target (versión previa)](/help/r-release-notes/target-release-notes.md) | Se han actualizado las notas de la [!DNL Target Standard/Premium] versión 20.5.1 (17 de junio de 2020) para incluir información sobre la compatibilidad con A4T en [!DNL Analysis Workspace]. |
...
| Mayo de 21 | [Nodos de borde de Destinatario de lista de permitidos](/help/c-implementing-target/c-considerations-before-you-implement-target/allowlist-edges.md) | Se añadió `mboxedge30.tt.omtrdc.net` a la lista. |
...
| Abril de 2 | [Glosario de perfiles y variables](/help/c-target/c-visitor-profile/variables-profiles-parameters-methods.md) | Se ha añadido información sobre el uso de `user.header('x-forwarded-for')` con bordes AWS más recientes para recuperar las direcciones IP de los usuarios. |
...
| 20 de febrero | [targetGlobalSettings()](/help/c-implementing-target/c-implementing-target-for-client-side-web/targetgobalsettings.md) | Se ha actualizado la descripción de la configuración `enabled`. Información añadida para la siguiente configuración: `pageLoadEnabled` y `viewsEnabled`. |
|  | [Ubicación geográfica](/help/c-target/c-audiences/c-target-rules/geo.md) | Tenga en cuenta que `mboxOverride.browserIp` se admite en at.js 1.Solamente *x.* |
